console.log("script check second scene")
   ...
   if (checkIfFinished()) {
      ...
      fileObjArr.forEach(fileObj => {
         if (fileObj.filter(infoRow => infoRow[1] == 'winner').length != 0) {
            if (!(winningMarginData.some(winObj => winObj.country == fileObj.filter(infoRow => infoRow[1] == 'winner')[0][2]))) {
               winningMarginData.push(
                  {country: fileObj.filter(infoRow => infoRow[1] == 'winner')[0][2], 
                     runMargins: [], 
                     wicketMargins: [],
                     numOfWins: 1})
            } else {
               winningMarginData.find(winObj => winObj.country == fileObj.filter(infoRow => infoRow[1] == 'winner')[0][2]).numOfWins += 1
            }
            if (fileObj.filter(infoRow => infoRow[1] == 'winner_runs').length != 0) {
               winningMarginData.find(winObj => winObj.country == fileObj.filter(infoRow => infoRow[1] == 'winner')[0][2]).runMargins.push(fileObj.filter(infoRow => infoRow[1] == 'winner_runs')[0][2])
            } else if (fileObj.filter(infoRow => infoRow[1] == 'winner_wickets').length != 0) {
               winningMarginData.find(winObj => winObj.country == fileObj.filter(infoRow => infoRow[1] == 'winner')[0][2]).wicketMargins.push(fileObj.filter(infoRow => infoRow[1] == 'winner_wickets')[0][2])
            }
         }
      });
      console.log("winner countries and win counts: ", winningMarginData)

      winningMarginData.forEach(winObj => {
         if (winObj.runMargins.length != 0) {
            const maxFromIndAvg = winObj.runMargins.reduce((a, b) => +a + +b) / winObj.runMargins.length
      ...
      const x = d3.scaleBand()
         .domain(winningMarginData.map(winObj => winObj.country))
      ...
      svg.append("text")
         ...
         .attr("x", width /3)
         .attr("y", 20)
         .text("Average Margin of Victory for Countries");
      ...
      svg.append("text")
         ...
         .attr("x", (width / 3) + 60)
         .attr("y", 50)
         .text("From Cricket T20 World Cups");
      ...
      svg.append("g")
         ...
         .call(d3.axisLeft(yRuns));
      
      svg.append("text")
         .attr("class", "y label runs")
         .attr("text-anchor", "end")
         .attr("y", 150)
         .attr("x", -100)
         .attr("dy", "0.75em")
         .attr("transform", "rotate(-90)")
         .text("Averge Margin of Victory in Runs");
      ...
      svg.append("g")
         ...
         .attr("y", d => {
            if (d.runMargins.length != 0) {
               return yRuns(d.runMargins.reduce((a, b) => +a + +b) / d.runMargins.length)
            } else {
               return yRuns(0)
            }
         })
         ...
         .attr("height", d => {
            if (d.runMargins.length != 0) {
               return yRuns(0) - yRuns(d.runMargins.reduce((a, b) => +a + +b) / d.runMargins.length)
            } else {
               return 0
            }
         })
